...
export function AddIngredientForm({ onAddIngredient }: Props) {
  ...
  async function handleAdd(e: FormEvent) {
    ...
    const result = await onAddIngredient(item.id, qty, unit);
    setPending(false);
    if (result.error) {
      setError(result.error);
      return;
        ...
        <div className="flex flex-col gap-2">
          ...
          <form onSubmit={handleAdd} className="flex gap-2">
            <input
              ...
              className="w-24 rounded-lg border border-black/10 bg-white px-4 py-2 text-sm outline-none focus:border-black/30 dark:border-white/15 dark:bg-white/5 dark:focus:border-white/30"
            />
            <input
              type="text"
              value={unit}
              ...
              className="w-32 rounded-lg border border-black/10 bg-white px-4 py-2 text-sm outline-none focus:border-black/30 dark:border-white/15 dark:bg-white/5 dark:focus:border-white/30"
            />
            <button
              type="submit"
              disabled={pending}
